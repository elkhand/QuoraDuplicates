...
class GRUCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable("W_r", [self.state_size, self.state_size], initializer=xavier_initializer)
            U_r = tf.get_variable("U_r", [self.input_size, self.state_size], initializer=xavier_initializer)
            b_r = tf.get_variable("b_r", [self.state_size], initializer=zero_initializer)
            W_z = tf.get_variable("W_z", [self.state_size, self.state_size], initializer=xavier_initializer)
            U_z = tf.get_variable("U_z", [self.input_size, self.state_size], initializer=xavier_initializer)
            b_z = tf.get_variable("b_z", [self.state_size], initializer=zero_initializer)
            W_o = tf.get_variable("W_o", [self.state_size, self.state_size], initializer=xavier_initializer)
            U_o = tf.get_variable("U_o", [self.input_size, self.state_size], initializer=xavier_initializer)
            b_o = tf.get_variable("b_o", [self.state_size], initializer=zero_initializer)

            z_t = tf.sigmoid(tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z)
            r_t = tf.sigmoid(tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r)
            o_t = tf.tanh(tf.matmul(inputs, U_o) + tf.matmul(tf.mul(r_t, state), W_o) + b_o)
            new_state = tf.mul(z_t, state) + tf.mul(1.0 - z_t, o_t)
            ### END YOUR CODE ###
