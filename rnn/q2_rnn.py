...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        feat_sent = [zero_vector] * max_length
        feat_lab = [zero_label] * max_length
        feat_mask = [False] * max_length
        for i, word in enumerate(sentence):
            ...
            feat_lab[i] = labels[i]
            feat_mask[i] = True
        ret.append((feat_sent, feat_lab, feat_mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        pred_pack = tf.pack(preds)
        preds = tf.transpose(pred_pack, perm=[1,0,2])
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(preds_masked, labels_masked))
        ### END YOUR CODE
        return loss
