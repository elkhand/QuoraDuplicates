...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable("W_r", [self._state_size, self._state_size], initializer=xavier_init)
            U_r = tf.get_variable("U_r", [self.input_size, self._state_size], initializer=xavier_init)
            b_r = tf.get_variable("b_r", initializer =np.array(np.zeros([1, self._state_size]), dtype=np.float32))
            W_z = tf.get_variable("W_z", [self._state_size, self._state_size], initializer=xavier_init)            
            U_z = tf.get_variable("U_z", [self.input_size, self._state_size], initializer=xavier_init)
            b_z = tf.get_variable("b_z", initializer =np.array(np.zeros([1, self._state_size]), dtype=np.float32))
            W_o = tf.get_variable("W_o", [self.state_size, self._state_size], initializer=xavier_init)
            U_o = tf.get_variable("U_o", [self.input_size, self._state_size], initializer=xavier_init)
            ...
            # z_t = tf.maximum(tf.sign(tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z), tf.zeros([self._state_size]))
            # r_t = tf.maximum(tf.sign(tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r), tf.zeros([self._state_size]))
            # o_t = 2* tf.maximum(tf.sign(tf.matmul(inputs, U_o) + tf.matmul(r_t*state, W_o) + b_o), tf.zeros([self._state_size])) - 1
